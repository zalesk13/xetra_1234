...
class TestS3BucketConnectorMethods(unittest.TestCase):
    """ 
    Testing the S3BucketConnectorClass
    """
    ...
    def test_list_files_in_prefix_wrong_prefix(self):
        """
        Tests the list_files_in_prefix_wrong_prefix method in case of a 
    ...
    def test_write_df_to_s3_csv(self):
        """
        Tests thw write_df_to_s3 method
    ...
    def test_write_df_to_s3_parquet(self):
        ...
        log_exp = f'Writing filte to {self.s3_endpoint_url}/{self.s3_bucket_name}/{key_exp}'
        # Test init
        file_format = 'parquet'
        # Method Execution
        with self.assertLogs() as logm:
    ...
    def test_write_df_to_s3_wrong_format(self):
        """ 
        Tests the write_df_to_s3 method
        if a non supported format is given as an argument
        """
        # Expected results
        df_exp = pd.DataFrame([['A', 'B'], ['C', 'D']], columns = ['col1', 'col2'])
        key_exp = 'test.parquet'
