...
class TestMetaProcessMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_s3 = moto.mock_s3() #mock_s3()
        self.mock_s3.start()
        ...
        # Creating a S3BucketConnectorInstance
        self.s3_bucket_meta = S3BucketConnector(self.s3_access_key,
                                                self.s3_secret_key,
                                                self.s3_endpoint_url,
                                                self.s3_bucket_name)
        self.dates = [(datetime.today().date() - datetime.timedelta(days=day))\
    ...
    def test_update_meta_file_no_meta_file(self):
        ...
        proc_date_list_result = list(
            pd.to_datetime(df_meta_result[MetaProcessFormat.META_SOURCE_DATE_COL.value])\
            .dt.date
        )
    ...
    def test_update_meta_file_empty_date_list(self):
        ...
        with self.AssertLogs() as logm:
            result = MetaProcess.update_meta_file(date_list, meta_key, self.s3_bucket_name)
            # Log test after method execution
            self.assertIn(log_exp, result)
        # Test after method execution
    ...
    def test_update_meta_file_meta_file_ok(self):
        """ 
        Tests the update_meta_file method
        when there already exists a meta file
        """
        # Expected results
        ...
        meta_content = (
            ...
            f'{date_list_old[1]},'
            f'{datetime.today().strftime(MetaProcessFormat.META_PROCESS_DATE_FORMAT)}'
        )
        ...
        # Read Meta File
        data = self.s3_bucket.put_objcet(Body=meta_content, Key=meta_key)
        out_buffer = StringIO(data)
        df_meta_result = pd.read_csv(out_buffer)
    ...
    def test_return_date_list_meta_file_wrong(self):
        """ 
        Tests the return_date_list method
        when there is a wrong mneta file
        """
        # Test init
        meta_key = 'meta.csv'
        meta_content = (
