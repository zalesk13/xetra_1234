...
class XetraSourceConfig(NamedTuple):
    ...
    src_first_extract_date: dertermines the date for extracting the source
    src_columns: source column names
    src_col_date: column names for date in source
...
class XetraTargetConfig(NamedTuple):
    ...
    trg_col_clos_price: column name for closing prioce in target
    ...
    trg_col_ch_prev_clos: column name for chnage to previous day's closing price in target
...
class XetraETL():
    """
    Reads the Xetra data, transforms and writes the transformed data to the target
    """
    ...
    def __init__(self, s3_bucket_src: S3BucketConnector,
        ...
        :param meta_key: used as a self.meta_key -> key of meta file
    ...
    def extract(self):
        """ 
        Read the source data and concatenates them to a single Pandas data frame
    ...
    def transform_report1(self, data_frame: pd.DataFrame):
        ...
        data_frame[self.trg_args.trg_col_ch_prev_clos] = \
            (data_frame[self.trg_args.trg_col_op_price] - data_frame[self.trg_args.trg_col_clos_price]) / data_frame[self.trg_args.trg_col_clos_price] * 100
        # Roun ding to 2 decimals
        data_frame = data_frame.round(decimals=2)
        # Remopving the day before extract_date
